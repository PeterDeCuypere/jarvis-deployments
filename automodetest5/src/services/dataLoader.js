...
export async function loadDataFile() {
  ...
  // Log first 500 chars for debugging - use JSON.stringify to see actual content
  console.log('[DataLoader] First 500 chars (raw):', JSON.stringify(csvText.substring(0, 500)));

  // Log first 50 char codes to see if there's any prefix/BOM
  const first50Codes = Array.from(csvText.substring(0, 50)).map(c => c.charCodeAt(0));
  console.log('[DataLoader] First 50 char codes:', first50Codes);
  console.log('[DataLoader] First 50 as hex:', first50Codes.map(c => c.toString(16).padStart(2, '0')).join(' '));
  ...
  console.log('[DataLoader] Number of lines:', lines.length);
  console.log('[DataLoader] First line (JSON escaped):', JSON.stringify(firstLine.substring(0, 200)));
  console.log('[DataLoader] First line length:', firstLine.length);
  console.log('[DataLoader] First line char codes (first 20):', Array.from(firstLine.substring(0, 20)).map(c => c.charCodeAt(0)));
  console.log('[DataLoader] Contains comma?', firstLine.includes(','));
  console.log('[DataLoader] Number of commas:', (firstLine.match(/,/g) || []).length);

  // Also check second line in case first line has BOM or prefix
  if (lines.length > 1) {
    const secondLine = lines[1] || '';
    console.log('[DataLoader] Second line (JSON escaped):', JSON.stringify(secondLine.substring(0, 200)));
    console.log('[DataLoader] Second line commas:', (secondLine.match(/,/g) || []).length);
  }

  // More robust CSV detection - check for common CSV patterns
  const hasCommas = trimmedFirstLine.includes(',');
  const looksLikeHeader = /^[a-zA-Z_][a-zA-Z0-9_,\s]*$/.test(trimmedFirstLine.replace(/,/g, '').replace(/\s/g, ''));
  const hasMultipleLines = lines.length > 1;

  console.log('[DataLoader] Has commas:', hasCommas);
  console.log('[DataLoader] Looks like header:', looksLikeHeader);
  console.log('[DataLoader] Has multiple lines:', hasMultipleLines);

  // If we have commas OR multiple lines with data, consider it valid CSV
  if (!hasCommas && !hasMultipleLines) {
    console.error('[DataLoader] First line does not contain commas:', trimmedFirstLine);
    console.error('[DataLoader] Raw first 100 bytes hex:', Array.from(csvText.substring(0, 100)).map(c => c.charCodeAt(0).toString(16).padStart(2, '0')).join(' '));
