...
function ModeTimelineChart() {
  const { analysisResults, rawData } = useAppStore();
  ...
  // Prepare mode duration statistics
  const durationStats = useMemo(() => {
    ...
    return Object.entries(analysisResults.modeStatistics)
      .map(([mode, stats]) => ({
        ...
        totalDurationFormatted: formatDuration(stats.totalDurationMs),
  ...
  // Plotly bar chart trace - use time in hours for visual, custom text for hover
  const barTrace = useMemo(() => {
    if (durationStats.length === 0) return null;
    // Convert ms to hours for bar chart display
    const hoursData = durationStats.map(d => (d.totalDurationMs || 0) / (1000 * 60 * 60));
    return {
      type: 'bar',
      ...
      text: durationStats.map(d => d.totalDurationFormatted),
  ...
  return (
    <div className="space-y-6">
            ...
            {durationStats.map((mode, idx) => {
              ...
              return (
                <div
                  ...
                  title={`Mode ${mode.modeId}: ${mode.totalDurationFormatted} (${mode.percentage.toFixed(1)}%)`}
                >
                  ...
                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-3 px-3 py-2 bg-gray-900 text-white text-xs rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none border border-gray-700 whitespace-nowrap">
                    ...
                    <div>Mode Duration: {mode.totalDurationFormatted}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          <h3 className="font-semibold mb-4">Mode Duration</h3>
          <div className="h-64">
            {barTrace && (
              <Plot
                data={[barTrace]}
                layout={{
                  ...
                  height: 250,
                  xaxis: {
                    title: { text: 'Hours', font: { size: 11, color: '#a3a3a3' } },
