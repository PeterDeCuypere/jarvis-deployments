...
function TimeSeriesChart() {
  ...
  // Filter selected pairs and output vars
  ...
  const chartData = useMemo(() => {
    ...
    return rawData
      .map((row, idx) => {
        const point = {
          index: idx,
          timestamp: selectedTimestampCol ? row[selectedTimestampCol] : idx
        };
  ...
  }, [rawData, selectedTimestampCol, selectedPairs, selectedOutputVars]);
  ...
  // Build list of all available variables for the selector
  const allVariables = useMemo(() => {
    const vars = [];
    selectedPairs.forEach(pair => {
      vars.push({
        name: `${pair.baseName}_SP`,
        label: `${pair.baseName} SP`,
        type: 'sp',
        baseName: pair.baseName
      });
      vars.push({
        name: `${pair.baseName}_PV`,
        label: `${pair.baseName} PV`,
        type: 'pv',
        baseName: pair.baseName
      });
    });
    selectedOutputVars.forEach(varName => {
      vars.push({ name: varName, label: varName, type: 'output' });
    });
    return vars;
  }, [selectedPairs, selectedOutputVars]);

  // Determine which variables to show based on selection
  const visiblePairs = useMemo(() => {
    if (!selectedVariable) return selectedPairs;
    const selected = allVariables.find(v => v.name === selectedVariable);
    if (!selected || selected.type === 'output') return [];
    return selectedPairs.filter(p => p.baseName === selected.baseName);
  }, [selectedVariable, selectedPairs, allVariables]);

  const visibleOutputVars = useMemo(() => {
    if (!selectedVariable) return selectedOutputVars;
    const selected = allVariables.find(v => v.name === selectedVariable);
    if (!selected || selected.type !== 'output') return [];
    return [selected.name];
  }, [selectedVariable, selectedOutputVars, allVariables]);
  ...
  const spPvTraces = useMemo(() => {
    ...
    const tagsToShow = visiblePairs.filter(pair => checkedTags.has(pair.baseName));
    ...
    tagsToShow.forEach((pair) => {
      ...
      traces.push({
        ...
        hovertemplate: `%{x|%Y-%m-%d %H:%M:%S}<br>${pair.baseName} SP: %{y:.2f}<extra></extra>`,
      ...
      traces.push({
        ...
        hovertemplate: `%{x|%Y-%m-%d %H:%M:%S}<br>${pair.baseName} PV: %{y:.2f}<extra></extra>`,
        showlegend: false // Hide from Plotly legend - we use custom checkboxes
      });
    });
    ...
    return traces;
  }, [chartData, visiblePairs, checkedTags, selectedPairs]);
  ...
  const outputTraces = useMemo(() => {
    ...
    const varsToShow = visibleOutputVars.filter(varName => checkedOutputVars.has(varName));
    ...
    return varsToShow.map((varName) => ({
      ...
      hovertemplate: `%{x|%Y-%m-%d %H:%M:%S}<br>${varName}: %{y:.2f}<extra></extra>`,
      showlegend: false // Hide from Plotly legend - we use custom checkboxes
    }));
  }, [chartData, visibleOutputVars, checkedOutputVars, selectedOutputVars]);
    ...
    <div className="bg-card rounded-xl border border-border p-4 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-1 flex-wrap">
          {/* Variable Selector for Mode Visualization */}
          {hasModes && allVariables.length > 0 && (
            <select
              value={selectedVariable || ''}
              onChange={(e) => setSelectedVariable(e.target.value || null)}
              className="px-3 py-1.5 text-sm bg-secondary border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary mr-2"
            >
      ...
      {/* Hint for mode visualization */}
      {hasModes && !selectedVariable && (
        <p className="text-xs text-muted-foreground bg-secondary/50 px-3 py-2 rounded-lg">
          Select a single variable from the dropdown to see mode regions clearly overlaid on the data.
        </p>
      )}
      ...
      {visiblePairs.length > 0 && (
        <div>
          <p className="text-sm text-muted-foreground mb-2">
            {selectedVariable ? `Selected: ${selectedVariable.replace('_', ' ')}` : 'Setpoints & Process Values'}
          </p>
      ...
      {visibleOutputVars.length > 0 && (
        <div>
          <p className="text-sm text-muted-foreground mb-2">
            {selectedVariable && !selectedVariable.includes('_') ? `Selected: ${selectedVariable}` : 'Output Variables'}
          </p>
