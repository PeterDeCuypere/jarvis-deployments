...
class DatabaseService {
  ...
  getSureBeamStats() {
    ...
    try {
      ...
      // Find date column - look for columns containing 'date' or 'pdate'
        ...
        try {
          ...
          if (result.length > 0 && result[0].values[0][0]) {
            stats.yearRange = {
              ...
              max: parseInt(result[0].values[0][1])
            };
          }
        } catch (e) {
      ...
      // Get PCN count by year - fetch ALL years (no LIMIT)
      if (tableNames.includes('pcn') && dateColumn) {
        try {
          const result = this.db.exec(`
          ...
          if (result.length > 0 && result[0].values.length > 0) {
            stats.pcnByYear = result[0].values
              .filter(row => row[0] && parseInt(row[0]) >= 2000)  // Filter out invalid years
              .map(row => ({
                year: row[0],
                count: row[1]
              }));
  ...
  getDoseOptimizationData(filters = {}) {
    ...
    try {
      ...
      // 5. Yearly trends - PCN count by year with dose info
      if (tableNames.includes('pcn') && dateColumn) {
        try {
      ...
      if (result.spsaDetails.length > 0) {
        ...
        const marginsFromDetails = result.spsaDetails
          ...
          .map(s => ((s.dose_range || 0) / s.dose_min) * 100);

        if (marginsFromDetails.length > 0) {
          const avgMarginPercent = marginsFromDetails.reduce((sum, m) => sum + m, 0) / marginsFromDetails.length;

          // Estimate: If we can tighten margins by half, energy savings potential
          const potentialReduction = avgMarginPercent / 2;
          // Energy savings correlates with dose reduction - roughly 0.5-0.8x
          const estimatedEnergySavings = potentialReduction * 0.6;
          ...
          result.savingsEstimate = {
            currentAvgMargin: avgMarginPercent,
            potentialMarginReduction: potentialReduction,
            estimatedEnergySavingsPercent: estimatedEnergySavings,
            spsasWithWideMargin: overDose,
            optimizationPotential: total > 0 ? (overDose / total) * 100 : 0,
  ...
  getThroughputOptimizationData(filters = {}) {
    ...
    try {
      ...
      // 4. Get daily throughput patterns (by day of week)
      if (tableNames.includes('pcn') && startDateCol) {
        try {
      ...
      // 7. Calculate peak capacity estimate (max jobs per day)
      if (tableNames.includes('pcn') && startDateCol) {
        try {
